...
# generate 1000 random samplings of circ and sj reads for each sample and calculate the circRNA and SJ expression as the average of the 1000 random samplings
realC=data_DS2$circData_DS2$circCounts_filter[, -infocolsC]
realS=data_DS2$sjData_DS2$sjmaxCounts_filter[, -infocolsS]

# Note: "Sample" randomly samples from "Real"
sampleC=data_DS2$circData_DS2$circCounts_filter[, -infocolsC]
sampleS=data_DS2$sjData_DS2$sjmaxCounts_filter[, -infocolsS]
...
clr_sample=data.frame(clr_ds2[, infocolsC], clr_sample)

#############################Calculate major isoform relative clr
######Random sampling data
# calculate mean CLR across CTl samples for the random sampling and real data
clr_sample$MeanCTL=apply(clr_sample[, rownames(samples2[which(samples2$ASD.CTL=="CTL"),])] ,1,mean)
...
########Real data
for (j in c((1+length(infocolsC)):ncol(clr_ds2))) 
{
  clr_ds2[which(is.na(clr_ds2[,j])==TRUE),j]=0
  clr_ds2[which(clr_ds2[,j]==Inf),j]=1
}
clr_ds2$MeanCTL=apply(clr_ds2[, rownames(samples2[which(samples2$ASD.CTL=="CTL"),])] ,1,mean)
na1=grep("NA", clr_ds2$EnsID)
clr_ds2_agMax=aggregate(clr_ds2$MeanCTL[-na1] ~ clr_ds2$EnsID[-na1], FUN = max)
clr_ds2_agSum=aggregate(clr_ds2$MeanCTL[-na1] ~ clr_ds2$EnsID[-na1], FUN = sum)
# Match the order of circRNAs in the CLR dataframes, and the nclr_per_gene vector
nclr_per_gene=table(clr_ds2$EnsID[-na1])
clr_ds2_agMax=clr_ds2_agMax[match(names(nclr_per_gene), clr_ds2_agMax[,1]), ]
clr_ds2_agSum=clr_ds2_agSum[match(names(nclr_per_gene), clr_ds2_agSum[,1]), ]
# Calculate major isoform relative CLR
major_isoform_ratio_clr_ds2=clr_ds2_agMax[,2]/clr_ds2_agSum[,2]

############Plot 
real=data.frame( nclr_per_gene,major_isoform_ratio_clr_ds2, "real" )
...
plotdata$categ=ifelse (plotdata$categ%in%"real", "DS2", "Random Sampling")

pdf("Analysis/Fig3/Fig3.MajorIsoform_with_RandomSampling_DS2_1000.pdf",height=4, width=10 )
